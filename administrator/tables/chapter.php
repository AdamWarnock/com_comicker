 ...
 * chapter Table class
 */
class ComickerTablechapter extends JTable
{
	...
	{
		parent::__construct('#__comicker_chapters', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if(($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state','com_comicker') && $array['state'] == 1)){
			$array['state'] = 0;
		}
		if($array['id'] == 0){
		...
		if ( isset($array['chaptertags']) && !empty($array['chaptertags']) ) {
			...
			if ( is_array($array['chaptertags']) ) {
				...
				foreach ($array['chaptertags'] as $singleTag) {
				...
				$array['chaptertags'] = implode(',',$tags);
			}
		}
		else {
			$array['chaptertags'] = '';
		...
		//Support for multiple or not foreign key field: parentcomic
			if(!empty($array['parentcomic'])){
				if(is_array($array['parentcomic'])){
					$array['parentcomic'] = implode(',',$array['parentcomic']);
				}
				else if(strrpos($array['parentcomic'], ',') != false){
					$array['parentcomic'] = explode(',',$array['parentcomic']);
				}
			}
			else {
				$array['parentcomic'] = '';
		...
		//Support for multiple or not foreign key field: parentchapter
			if(!empty($array['parentchapter'])){
				if(is_array($array['parentchapter'])){
					$array['parentchapter'] = implode(',',$array['parentchapter']);
				}
				else if(strrpos($array['parentchapter'], ',') != false){
					$array['parentchapter'] = explode(',',$array['parentchapter']);
				}
			}
			else {
				$array['parentchapter'] = '';
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_comicker.chapter.' . $array['id']))
		{
			$actions         = JFactory::getACL()->getActions('com_comicker', 'chapter');
			$default_actions = JFactory::getACL()->getAssetRules('com_comicker.chapter.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_comicker.chapter.' . (int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		$assetParent->loadByName('com_comicker');
