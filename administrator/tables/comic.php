 ...
 * comic Table class
 */
class ComickerTablecomic extends JTable
{
	...
	{
		parent::__construct('#__comicker_comics', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if ( isset($array['comictags']) && !empty($array['comictags']) ) {
			...
			if ( is_array($array['comictags']) ) {
				...
				foreach ($array['comictags'] as $singleTag) {
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_comicker.comic.' . $array['id']))
		{
			$actions         = JFactory::getACL()->getActions('com_comicker', 'comic');
			$default_actions = JFactory::getACL()->getAssetRules('com_comicker.comic.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_comicker.comic.' . (int) $this->$k;
	}
